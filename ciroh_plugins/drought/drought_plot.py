...
class DroughtDataTimeSeries(base.DataSource):
    ...
    visualization_description = "An interactive chart that depicts drought level percentage of areas within a specified domain"
    visualization_args = {
    ...
    def read(self):
        ...
        traces = (
            self.create_usdm_traces(data)
            if self.data_index == "usdm"
            else self.create_dsci_traces(data)
        )
    ...
    def create_layout():
        ...
        layout = {
            ...
            "xaxis": {"title": "Date", "tickformat": "%Y-%m-%d", "type": "date"},
            "yaxis": {"title": "Percentage Area", "ticksuffix": "%"},
            "legend": {"title": {"text": "Drought Level"}},
            "hovermode": "x",
